...
export default function AddLeadPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const ieltsScoreNum = parseFloat(formData.ieltsScore);
        ...
        body: JSON.stringify(
          isAgentPortal
            ? {
                ...
                lastAcademicQualification: formData.lastAcademicQualification || null,
                hasIelts: formData.hasIelts === "yes" ? true : formData.hasIelts === "no" ? false : null,
                ieltsScore: !isNaN(ieltsScoreNum) && formData.ieltsScore ? ieltsScoreNum : null,
              }
            : {
                firstName: formData.firstName.trim(),
                lastName: formData.lastName.trim(),
      ...
      if (!leadId) throw new Error("Lead created but no lead ID was returned");

      const destinationBase = isAgentPortal ? "/agent/leads" : "/dashboard/leads";
      router.push(`${destinationBase}/${leadId}`);
      router.refresh();
    } catch (err) {
      setErrors({ submit: err instanceof Error ? err.message : "Failed to create lead" });
    } finally {
      setLoading(false);
    }
  }
  ...
  const inputCls = (err?: string) =>
    cn("w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500", err ? "border-red-300 bg-red-50" : "border-slate-300");

  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-slate-900 mb-2">Add New Lead</h1>
        <p className="text-slate-600">Create a new lead entry in the system</p>
      </div>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <input type="text" value={formData.email} onChange={(e) => set("email", e.target.value)}
            ...
            <div>
              ...
              <div className="relative">
                <button type="button" onClick={() => setShowNationalityDropdown(!showNationalityDropdown)}
                  ...
                  <span className={formData.nationality ? "" : "text-slate-500"}>{formData.nationality || "Select nationality"}</span>
                ...
                {showNationalityDropdown && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-50">
                    <input type="text" placeholder="Search..." value={nationalitySearch}
                    ...
                    <div className="max-h-48 overflow-y-auto">
                      {filteredNationalities.length > 0 ? filteredNationalities.map((country) => (
                        <button key={country} type="button" onClick={() => { set("nationality", country); setShowNationalityDropdown(false); setNationalitySearch(""); }}
                          className="w-full text-left px-3 py-2 hover:bg-slate-100 text-sm">{country}</button>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Last Academic Qualification</label>
              <select value={formData.lastAcademicQualification} onChange={(e) => set("lastAcademicQualification", e.target.value)}
                className={inputCls()}>
            ...
            {/* Do you have IELTS? */}
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Do you have IELTS?</label>
              <select value={formData.hasIelts} onChange={(e) => { set("hasIelts", e.target.value); if (e.target.value !== "yes") set("ieltsScore", ""); }}
                className={inputCls()}>
            ...
            <div>
              ...
              <input type="number" step="0.5" min="0" max="9" value={formData.ieltsScore}
                onChange={(e) => set("ieltsScore", e.target.value)}
                disabled={formData.hasIelts !== "yes"}
                placeholder={formData.hasIelts === "yes" ? "e.g. 6.5" : "—"}
                className={cn(inputCls(), formData.hasIelts !== "yes" && "bg-slate-50 text-slate-400 cursor-not-allowed")} />
            </div>

            {/* Source */}
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Source *</label>
              <select value={formData.source} onChange={(e) => set("source", e.target.value)}
            ...
            {/* Interested In */}
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Interested In (Course Level)</label>
              <select value={formData.interestedIn} onChange={(e) => set("interestedIn", e.target.value)}
                className={inputCls()}>
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <select value={formData.dialCode} onChange={(e) => set("dialCode", e.target.value)}
                  className="w-24 px-2 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                  {DIAL_CODES.map((dc) => (
                    <option key={dc.code} value={dc.code}>{dc.code}</option>
                  ))}
                </select>
                <input type="text" value={formData.phone}
                  onChange={(e) => set("phone", e.target.value.replace(/\D/g, ""))}
                  placeholder="9876543210"
                  className={cn("flex-1 px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500", errors.phone ? "border-red-300 bg-red-50" : "border-slate-300")} />
              </div>
            ...
            <div>
              ...
              <div className="relative">
                <button type="button" onClick={() => setShowCountryDropdown(!showCountryDropdown)}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg text-left flex items-center justify-between hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-500">
                  <span className={formData.countryOfResidence ? "" : "text-slate-500"}>{formData.countryOfResidence || "Select country"}</span>
                  <ChevronDown className="w-4 h-4 text-slate-400" />
                </button>
                {showCountryDropdown && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-50">
                    ...
                    <div className="max-h-48 overflow-y-auto">
                      {filteredCountries.length > 0 ? filteredCountries.map((country) => (
                        <button key={country} type="button" onClick={() => { set("countryOfResidence", country); setShowCountryDropdown(false); setCountrySearch(""); }}
                          className="w-full text-left px-3 py-2 hover:bg-slate-100 text-sm">{country}</button>
                      )) : <div className="px-3 py-2 text-sm text-slate-500">No results</div>}
                    </div>
                  </div>
            ...
            {/* Assigned Counsellor */}
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Assigned Counsellor</label>
              <select value={formData.assignedCounsellorId}
                onChange={(e) => set("assignedCounsellorId", e.target.value)}
              ...
              {user.roleName === "COUNSELLOR" && (
                <p className="text-xs text-slate-500 mt-1">Automatically assigned to you</p>
              )}
            </div>

            {/* Sub-Agent */}
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Sub-Agent (Optional)</label>
              <select value={formData.subAgentId} onChange={(e) => set("subAgentId", e.target.value)}
                className={inputCls()}>
            ...
            <div>
              ...
              <select value={formData.preferredDestination} onChange={(e) => set("preferredDestination", e.target.value)}
                className={inputCls()}>
