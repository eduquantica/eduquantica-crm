...
interface CourseData {
  ...
  university: { id: string; name: string; country: string; currency: string };
}
...
function parseIntakeDates(raw: unknown): IntakeDate[] {
...
export default function EditCourseForm({ course, universities }: EditCourseFormProps) {
  ...
  // Pre-populate form state from existing course
  const [name, setName] = useState(course.name);
  const [level, setLevel] = useState(course.level);
  const [fieldOfStudy, setFieldOfStudy] = useState(course.fieldOfStudy);
  const [duration, setDuration] = useState(course.duration ?? "");
  const [studyMode, setStudyMode] = useState(course.studyMode);
  const [tuitionFee, setTuitionFee] = useState(
    course.tuitionFee != null ? String(course.tuitionFee) : ""
  );
  const [applicationFee, setApplicationFee] = useState(
    course.applicationFee != null ? String(course.applicationFee) : ""
  );
    ...
    try {
      ...
      const res = await fetch(`/api/admin/courses/${course.id}`, {
        ...
        body: JSON.stringify({
          ...
          tags: selectedTags,
          intakeDatesWithDeadlines: intakeDates.length > 0 ? intakeDates : [],
          isActive,
        }),
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Link href={`/dashboard/courses/${course.id}`} className="p-2 hover:bg-gray-100 rounded-md">
          <ArrowLeft size={20} />
        </Link>
        <div>
        ...
        <div className="space-y-6 rounded-lg border border-gray-200 bg-white p-6">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div>
              ...
              <div className="w-full rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-gray-700 text-sm">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <select
                  value={COMMON_FIELDS_OF_STUDY.includes(fieldOfStudy) ? fieldOfStudy : ""}
              ...
              {!COMMON_FIELDS_OF_STUDY.includes(fieldOfStudy) && fieldOfStudy && (
                <p className="mt-1 text-xs text-gray-500">
                  Current: <span className="font-medium">{fieldOfStudy}</span>
                </p>
              )}
              {showFieldInput && (
