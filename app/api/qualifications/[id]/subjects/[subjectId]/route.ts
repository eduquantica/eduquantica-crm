...
export async function PATCH(
  ...
  let cleanedGrade = "";
  try {
    cleanedGrade = normaliseGrade(payload.data.grade, payload.data.gradeType);
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Invalid grade" },
      { status: 400 },
  ...
  const updated = await db.studentSubjectGrade.updateMany({
    where: {
      id: params.subjectId,
      qualificationId: qualification.id,
    },
    data: (({
      rawGrade: cleanedGrade,
      gradeType: payload.data.gradeType,
      universalScore: GradeNormaliser.normalise(cleanedGrade, qualification.qualType),
      isConfirmedByStudent: true,
    } as unknown) as never),
  });
  ...
  const subject = await db.studentSubjectGrade.findUnique({
...
export async function DELETE(
  ...
  const deleted = await db.studentSubjectGrade.deleteMany({
    where: {
      id: params.subjectId,
      qualificationId: qualification.id,
    },
