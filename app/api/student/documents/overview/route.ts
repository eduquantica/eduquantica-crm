...
import { localUploadKeyFromUrl } from "@/lib/local-upload";
...
function statusLabel(status: string, scanDecision: string | null, scanStatus: string | null) {
...
export async function GET() {
  ...
  smartChecklist.push({
    ...
    itemKind: "PASSPORT",
    status: passportDoc ? itemStatusFromDocument(passportDoc) : (student.passportNumber ? "UPLOADED" : "TODO"),
  ...
  for (const qualification of student.academicProfile?.qualifications || []) {
    ...
    const transcriptLabel = labelPrefix.includes("Degree") || labelPrefix === "PhD"
      ? `${labelPrefix} Transcript`
      : `${labelPrefix} Academic Transcript`;
    const certificateLabel = labelPrefix.includes("Degree") || labelPrefix === "PhD"
  ...
  for (const testScore of student.testScores) {
    ...
    const testTypeUpper = testTypeRaw.toUpperCase();
    let label = `English Test Certificate (${testTypeRaw})`;
    if (testTypeUpper.includes("IELTS")) label = `IELTS Certificate (${testTypeRaw})`;
    else if (testTypeUpper.includes("TOEFL")) label = "TOEFL Score Report";
    else if (testTypeUpper.includes("PTE")) label = "PTE Academic Certificate";
    else if (testTypeUpper.includes("DUOLINGO")) label = "Duolingo English Test Certificate";
    else if (testTypeUpper.includes("OET")) label = "OET Certificate";
    ...
    const fileName = testScore.certificateFileName || linkedDoc?.fileName || null;
  ...
  const highestLevel = levels.includes("PHD")
    ? "PHD"
    : levels.includes("MASTERS")
      ? "MASTERS"
      : levels.includes("BACHELORS")
        ? "BACHELORS"
        : null;
  ...
  for (let i = 0; i < requiredReferences; i += 1) {
    const doc = lorDocs[i] || null;
    smartChecklist.push({
      id: `auto-reference-${i + 1}`,
      label: requiredReferences === 1 ? "Reference Letter" : `Reference Letter ${i + 1}`,
      documentType: "LOR",
  ...
  const hasWrittenDraft = student.writtenDocuments.length > 0;
  smartChecklist.push({
    id: "auto-sop",
    label: "Personal Statement or Statement of Purpose",
    documentType: "SOP",
    itemKind: "SOP",
    status: sopDoc ? itemStatusFromDocument(sopDoc) : (hasWrittenDraft ? "UPLOADED" : "TODO"),
    hasFile: Boolean(sopDoc || hasWrittenDraft),
    fileUrl: sopDoc ? toApiFilesPath(sopDoc.fileUrl) : (student.writtenDocuments[0]?.convertedPdfUrl ? toApiFilesPath(student.writtenDocuments[0].convertedPdfUrl) : null),
    fileName: sopDoc?.fileName || student.writtenDocuments[0]?.title || null,
    uploadedAt: sopDoc?.uploadedAt.toISOString() || student.writtenDocuments[0]?.updatedAt.toISOString() || null,
  });
  ...
  smartChecklist.push({
    ...
    fileName: cvDoc?.fileName || "CV Profile",
    uploadedAt: cvDoc?.uploadedAt.toISOString() || student.cvProfile?.updatedAt.toISOString() || null,
  });

  if (student.applications.length > 0) {
    const bankStatementDoc = firstByTypeMatching(DocumentType.FINANCIAL_PROOF, (doc) =>
      matchesToken(doc.fileName, "bank") || matchesToken(doc.fileName, "statement"),
    ) || latestByType(DocumentType.FINANCIAL_PROOF);
  ...
  const countries = new Set(student.applications.map((app) => app.university.country.toUpperCase()));
  const visaDoc = latestByType(DocumentType.VISA_DOCUMENT);
  if (Array.from(countries).some((country) => country.includes("UK") || country.includes("UNITED KINGDOM") || country === "GB")) {
    smartChecklist.push({
      id: "auto-uk-tb",
      ...
      staffNote: "Required for nationals from certain countries",
    });
  }

  if (Array.from(countries).some((country) => country.includes("CANADA"))) {
  ...
  if (Array.from(countries).some((country) => country.includes("AUSTRALIA"))) {
    smartChecklist.push({
      id: "auto-australia-oshc",
      label: "Overseas Student Health Cover (OSHC) Evidence",
      documentType: "VISA_DOCUMENT",
      itemKind: "OSHC",
      status: visaDoc ? itemStatusFromDocument(visaDoc) : "TODO",
      hasFile: Boolean(visaDoc),
  ...
  if (Array.from(countries).some((country) => country.includes("USA") || country.includes("UNITED STATES"))) {
  ...
  if (stillRequiredCount === 0 && pendingReviewCount > 0) {
  ...
  const uploadedDocuments = await Promise.all(student.documents.map(async (doc) => {
    const size = await fileSizeByUrl(doc.fileUrl);
    return {
      id: doc.id,
      ...
      fileSize: size,
      status: statusLabel(doc.status, doc.scanResult?.counsellorDecision || null, doc.scanResult?.status || null),
      source: "SMART_DOCUMENT_VAULT",
    };
  }));

  const externalUploads: Array<{
    id: string;
    documentName: string;
    documentType: string;
    fileUrl: string;
  ...
  for (const score of student.testScores) {
    if (!score.certificateUrl) continue;
    externalUploads.push({
      ...
      status: score.isVerified ? "VERIFIED" : "PENDING",
      source: "TEST_SCORES",
    });
  }

  for (const written of student.writtenDocuments) {
    if (!written.convertedPdfUrl) continue;
    externalUploads.push({
      ...
      status: written.status === "APPROVED" ? "VERIFIED" : "PENDING",
      source: "WRITE_SOP",
    });
  }

  const allUploadedDocuments = [...uploadedDocuments, ...externalUploads];

  const requiredChecklist = smartChecklist.map((item) => ({
    id: item.id,
    ...
    requestedByRole: item.requestedByRole || null,
  ...
  return NextResponse.json({
    data: {
      ...
      stillRequiredCount,
      readyBanner,
      uploadedDocuments: allUploadedDocuments,
      requiredChecklist,
    },
  });
}
