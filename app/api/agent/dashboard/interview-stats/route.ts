...
export async function GET() {
  ...
  if (session.user.roleName !== "SUB_AGENT" && session.user.roleName !== "ADMIN") {
  ...
  const [preCasRaw, visaRaw, preCasRequired, visaRequired] = await Promise.all([
    prismaWithInterview.preCasInterview.groupBy({
      by: ["outcome"],
      where: { ...whereScope, isRequired: true, outcome: { not: null } },
      _count: { _all: true },
    }),
    ...
    prismaWithInterview.preCasInterview.count({ where: { ...whereScope, isRequired: true } }),
    prismaWithInterview.visaInterview.count({ where: { ...whereScope, isRequired: true } }),
  ]);
