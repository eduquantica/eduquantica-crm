...
const PRE_CAS_STAGES: PreCasStage[] = ["BEFORE_OFFER", "AFTER_CONDITIONAL_OFFER", "DURING_CAS_ISSUE"];
...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const row = await prismaWithInterview.preCasInterview.upsert({
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const current = await prismaWithInterview.preCasInterview.upsert({
    where: { applicationId: params.id },
    create: { applicationId: params.id },
    update: {},
  });
  ...
  if ("stage" in payload) {
    ...
    } else if (typeof payload.stage === "string" && PRE_CAS_STAGES.includes(payload.stage as PreCasStage)) {
      updateData.stage = payload.stage as PreCasStage;
    }
  }
  ...
  const updated = await prismaWithInterview.preCasInterview.update({
    where: { applicationId: params.id },
    data: updateData,
  });
      ...
      changed.map((field) =>
        db.activityLog.create({
          data: {
            ...
            entityId: params.id,
            action: "pre_cas_interview_updated",
  ...
  if (notificationContext) {
    if (!current.isRequired && updated.isRequired) {
      await notifyInterviewRequired("PRE_CAS", notificationContext);
    }

    if (!sameValue(current.bookedDate, updated.bookedDate) && updated.bookedDate) {
      await notifyInterviewBooked("PRE_CAS", notificationContext, updated.bookedDate, updated.stage);
    }

    if (!sameValue(current.outcome, updated.outcome) && updated.outcome) {
      await notifyInterviewOutcome("PRE_CAS", notificationContext, updated.outcome, updated.rescheduledDate);
    }
  }
